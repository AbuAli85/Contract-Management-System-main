...
export async function getDashboardAnalytics(): Promise<DashboardAnalytics> {
  try {
    ...
    const { data: contracts, error } = await supabase.from("contracts").select("status, created_at")
    ...
    const contracts_this_month = contracts.filter(
      (c) => c.created_at && new Date(c.created_at) >= lastMonth
    ).length
