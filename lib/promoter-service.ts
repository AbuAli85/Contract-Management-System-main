...
export async function searchPromoters(filter: PromoterFilter = {}): Promise<PromoterWithStats[]> {
  try {
    ...
    if (filter.date_from) {
      promoters = promoters.filter(
        (p) => p.created_at && isAfter(parseISO(p.created_at), parseISO(filter.date_from!))
      )
    }

    if (filter.date_to) {
      promoters = promoters.filter(
        (p) => p.created_at && isBefore(parseISO(p.created_at), parseISO(filter.date_to!))
      )
    }
  ...
  try {
    ...
    return promoters.filter((promoter) => {
      ...
      return (
        ...
        (passportExpiry &&
          isBefore(passportExpiry, futureDate) &&
          isAfter(passportExpiry, new Date()))
      )
    })
  } catch (error) {
...
export async function getPromoterActivitySummary(promoterId: string) {
  try {
    ...
    const { data: recentContracts, error: recentError } = await supabase
      .from("contracts")
      .select(
        "id, created_at, status, first_party_name_en, second_party_name_en, contract_start_date, contract_end_date"
      )
      .eq("promoter_id", promoterId)
...
export async function getPromoterDocumentAlerts(daysAhead: number = 30) {
  try {
    ...
    const { data: promoters, error } = await supabase
      .from("promoters")
      .select(
        "id, name_en, name_ar, id_card_number, id_card_expiry_date, passport_expiry_date, email"
      )
      .or(
    ...
    const now = new Date()
    const idCardExpiring = (promoters || []).filter(
      (p) =>
        p.id_card_expiry_date &&
    ...
    const passportExpiring = (promoters || []).filter(
      (p) =>
        p.passport_expiry_date &&
 ...
 */
export async function validateUniqueIdCard(
  idCardNumber: string,
  excludeId?: string
): Promise<boolean> {
  try {
    let query = supabase.from("promoters").select("id").eq("id_card_number", idCardNumber)
