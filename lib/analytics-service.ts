...
export async function getContractAnalytics(
  ...
  try {
    ...
    const lastMonthContracts = contractsData.filter(
      (c) =>
    ...
    // Value calculations
    const contractsWithValue = filteredContracts.filter(
    ...
    // Status distribution
    const statusDistribution = filteredContracts.reduce(
      (acc, contract) => {
 ...
 */
export async function getPromoterAnalytics(
  timeRange?: AnalyticsTimeRange
  ...
  try {
    ...
    // Status distribution
    const statusDistribution = filteredPromoters.reduce(
      (acc, promoter) => {
    ...
    const averageContractsPerPromoter =
      total > 0 ? promoterContractCounts.reduce((sum, p) => sum + p.contractCount, 0) / total : 0
...
export async function getPartyAnalytics(timeRange?: AnalyticsTimeRange): Promise<PartyAnalytics> {
  try {
    ...
    const { data: contracts, error: contractsError } = await supabase
      .from("contracts")
      .select(
        "first_party_id, second_party_id, status, contract_value, contract_start_date, contract_end_date"
      )
    ...
    filteredParties.forEach((party) => {
      ...
      if ((crExpiry && crExpiry < now) || (licenseExpiry && licenseExpiry < now)) {
        ...
        const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000)
        if (
          (crExpiry && crExpiry < thirtyDaysFromNow) ||
          (licenseExpiry && licenseExpiry < thirtyDaysFromNow)
        ) {
          documentsExpiring++
        }
      }
      ...
      const partyContracts = contractsData.filter(
        (c) => c.first_party_id === party.id || c.second_party_id === party.id
      )
    ...
    const withContracts = filteredParties.filter((party) => {
      return contractsData.some(
        (c) => c.first_party_id === party.id || c.second_party_id === party.id
      )
    }).length

    // Type distribution
    const byType = filteredParties.reduce(
      (acc, party) => {
    ...
    // Status distribution
    const statusDistribution = filteredParties.reduce(
      (acc, party) => {
    ...
    const contractDistribution = filteredParties
      .map((party) => {
        const partyContracts = contractsData.filter(
          (c) => c.first_party_id === party.id || c.second_party_id === party.id
        )
    ...
    const contractsWithDuration = contractsData.filter(
      (c) => c.contract_start_date && c.contract_end_date
    )
...
export async function getAlertAnalytics(): Promise<AlertAnalytics> {
  try {
    ...
    const byPriority = notificationsData.reduce(
      (acc, notification) => {
    ...
    // Type distribution
    const byType = notificationsData.reduce(
      (acc, notification) => {
