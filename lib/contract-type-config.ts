...
export const CONTRACT_TYPE_CONFIGS: Record<string, ContractTypeConfig> = {
  ...
  internship: {
    ...
    optionalFields: ["basic_salary", "allowances", "working_hours", "special_terms"],
    templatePlaceholders: [
      "{{intern_name}}",
      "{{company_name}}",
      "{{position}}",
...
export const ENHANCED_CONTRACT_TYPE_CONFIGS: Record<string, EnhancedContractTypeConfig> = {
  ...
  "oman-part-time-makecom": {
    ...
    optionalFields: ["contract_end_date", "allowances", "overtime_rate", "special_terms"],
    templatePlaceholders: [
      "{{employee_name}}",
      "{{employer_name}}",
      "{{job_title}}",
...
): EnhancedContractTypeConfig | null {
...
export function generateContractWithMakecom(
  ...
  if (!contractConfig || !templateConfig) {
    return {
      ...
      validation: {
        isValid: false,
        errors: ["Contract type not configured for Make.com"],
        warnings: [],
      },
    }
  }
  ...
  const templateValidation = validateMakecomTemplateData(
    contractConfig.makecomTemplateId!,
    contractData
  )
