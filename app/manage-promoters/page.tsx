...
import { format, parseISO, differenceInDays, isPast, isAfter, isBefore, subDays, addDays } from "date-fns"
...
export default function ManagePromotersPage() {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      ...
      case 'warning':
        return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200"><Clock className="w-3 h-3 mr-1" />Warning</Badge>
      case 'critical':
  ...
  const getDocumentStatusBadge = (status?: string) => {
    switch (status) {
      case 'valid':
        return <Badge className="bg-green-100 text-green-800 border-green-200"><FileCheck className="w-3 h-3 mr-1" />Valid</Badge>
      case 'expiring':
        return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200"><FileWarning className="w-3 h-3 mr-1" />Expiring</Badge>
      case 'expired':
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {filteredPromoters.length === 0 ? (
          <Card className="py-10 text-center">
            ...
            <CardContent>
              <CardDescription>
                {searchTerm || statusFilter !== "all" || documentFilter !== "all" || contractFilter !== "all"
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the promoter "{promoterToDelete?.name_en}". 
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <Tabs defaultValue="general" className="w-full">
              ...
              <TabsContent value="general" className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm mt-1">{selectedPromoter.name_en}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium">ID Card Number</Label>
                      <p className="text-sm font-mono mt-1">{selectedPromoter.id_card_number}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium">Email</Label>
                      <p className="text-sm mt-1">
                        {selectedPromoter.email ? (
                          <a href={`mailto:${selectedPromoter.email}`} className="text-primary hover:underline">
                            {selectedPromoter.email}
                          </a>
                        ) : (
                    ...
                    <div>
                      ...
                      <p className="text-sm mt-1">{selectedPromoter.active_contracts_count || 0}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm mt-1" dir="rtl">{selectedPromoter.name_ar || 'Not provided'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-sm mt-1">
                        {selectedPromoter.phone ? (
                          <a href={`tel:${selectedPromoter.phone}`} className="text-primary hover:underline">
                            {selectedPromoter.phone}
                          </a>
                        ) : (
                          <span className="text-muted-foreground">Not provided</span>
                    ...
                    <div>
                      ...
                      <p className="text-sm mt-1">{selectedPromoter.total_contracts_count || 0}</p>
                    </div>
                  </div>
                </div>
                ...
                {selectedPromoter.address && (
                  <div>
                    ...
                    <p className="text-sm mt-1">{selectedPromoter.address}</p>
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="documents" className="space-y-4">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div>
                        ...
                        {selectedPromoter.id_card_expiry_date && (
                          <p className="text-xs text-muted-foreground mt-1">
                            {(() => {
                              const days = differenceInDays(parseISO(selectedPromoter.id_card_expiry_date), new Date())
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div>
                        ...
                        {selectedPromoter.passport_expiry_date && (
                          <p className="text-xs text-muted-foreground mt-1">
                            {(() => {
                              const days = differenceInDays(parseISO(selectedPromoter.passport_expiry_date), new Date())
                              if (days < 0) return `Expired ${Math.abs(days)} days ago`
                              if (days === 0) return 'Expires today'
                              if (days <= 30) return `Expires in ${days} days`
                              return `Valid for ${days} days`
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2">{getDocumentStatusBadge(selectedPromoter.document_status)}</div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="history" className="space-y-4">
                <div className="space-y-3">
                  ...
                  {selectedPromoter.updated_at && selectedPromoter.updated_at !== selectedPromoter.created_at && (
                    <div className="flex items-center gap-3 text-sm">
                      <Clock className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">Last Updated:</span>
                      <span>
                        {format(parseISO(selectedPromoter.updated_at), 'PPP')}
                      </span>
                    </div>
                  )}
                </div>
